...
{
    initialized = true;
    int free_chosen_tests = false;
    struct unittest *chosen_tests;
    int return_code;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--keep-going") == 0) {
            ...
            fprintf(stderr,
                    "\t--which\tChoose how many and which tests you want to run "
                    " by passing the total number of tests and then the indices "
                    "of the tests you would like to run\n");
            fprintf(stderr, "\t-h        \tPrint this message.\n");
            return EXIT_FAILURE;
        } else if (strcmp(argv[i], "--which") == 0) {
            if ((argc <= i + 1) || !sscanf(argv[i + 1], "%d", &n_tests)) {
                fprintf(stderr, "Unable to read total number of chosen tests.\n");
                return EXIT_FAILURE;
            }
            if (!(chosen_tests = (struct unittest *) malloc(n_tests * sizeof(*chosen_tests)))) {
                fprintf(stderr, "Memory error\n");
                return EXIT_FAILURE;
            }
            free_chosen_tests = true;
            for (int j = 0; j < n_tests; j++) {
                int argv_index = i + 2 + j;
                size_t tests_index;
                if ((argc <= argv_index) || !sscanf(argv[argv_index], "%lu", &tests_index)) {
                    fprintf(stderr, "Unable to read test number %d. Instead found %s\n", argv_index, argv[argv_index]);
                    return EXIT_FAILURE;
                }
                chosen_tests[j] = tests[tests_index];
            }
            tests = chosen_tests;
            i += n_tests + 1;
        } else {
    ...
    return_code = run_tests(&c, tests, n_tests);
    
    if (free_chosen_tests) {
        free(chosen_tests);
    }
