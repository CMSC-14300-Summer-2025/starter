 ...
 */
void validate_student(const struct student *student);
 ...
 */
void write_student(const struct student *student, FILE *file);
 ...
 * the list of majors, this function returns -1.
 */
enum grade grade_number(const char *grade_name);

/* Convert the number of a major to its name. If the number does not correspond
 * to a major, "UNKNOWN" is returned.
 */
const char *grade_name(enum grade grade);
...
void validate_student(const struct student *s)
{
    ...
    if (!(s->grade < GRADE_COUNT)) {
        fprintf(stderr, "invalid grade `%d`. There are a total of %d possible grades\n", s->grade, GRADE_COUNT);
        abort();
    }
}
...
bool read_student(FILE *file, struct student *student_p)
{
     ...
     * correctly. */
    int n_read = fscanf(file, "%u\t%hhu\t%hhi\t%s\t%63[^\t]\t%17[^\n]\n",
            &s.ucid,
            &s.late_days,
            &s.point_adjust,
            grade_name_buf,
    ...
    enum grade g = grade_number(grade_name_buf);
    if (m < 0) {
        fprintf(stderr, "grade '%s' does not exist\n", grade_name_buf);
        exit(EXIT_FAILURE);
    }
...
enum grade grade_number(const char *grade_name) {
    if (strcmp(grade_name, "A") == 0) {
        return A;
    } else if (strcmp(grade_name, "A-") == 0) {
        return A_MINUS;
    } else if (strcmp(grade_name, "A-") == 0) {
        return A_MINUS;
    } else if (strcmp(grade_name, "B+") == 0) {
        return B_PLUS;
    } else if (strcmp(grade_name, "B") == 0) {
        return B;
    } else if (strcmp(grade_name, "B-") == 0) {
        return B_MINUS;
    } else if (strcmp(grade_name, "C+") == 0) {
        return C_PLUS;
    } else if (strcmp(grade_name, "C") == 0) {
        return C;
    } else if (strcmp(grade_name, "C-") == 0) {
        return C_MINUS;
    } else if (strcmp(grade_name, "D+") == 0) {
        return D_PLUS;
    } else if (strcmp(grade_name, "D") == 0) {
        return D;
    } else if (strcmp(grade_name, "F") == 0) {
        return F;
    } else if (strcmp(grade_name, "P") == 0) {
        return P;
    } else {
        return -1;
    }
